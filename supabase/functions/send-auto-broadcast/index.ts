...
Deno.serve(async (req) => {
  ...
  // Edge: если в невисокосный год нацелились на 28 февраля, ловим ещё и людей с 29 февраля.
  ...
  // 3. Клиенты с подходящим ДР. Грузим всех клиентов и фильтруем в памяти —
  // для текущей шкалы (десятки-сотни клиентов) хватает. Если выйдет на тысячи,
  // сделаем RPC с to_char(birth_date, 'MM-DD') = ...
  ...
  for (const r of recipients) {
    // 5. ANTI-SPAM: пытаемся вставить run-row. Если ON CONFLICT — этому уже шли сегодня.
    const { data: claim, error: claimErr } = await sb
      .from('auto_broadcast_runs')
      .insert({ auto_id: auto.id, recipient_id: r.id })
      .select('id').maybeSingle()
    if (claimErr || !claim) {
      // Может быть уже UNIQUE-конфликт — Supabase возвращает это error'ом,
      // не считаем за сбой, просто пропускаем.
      alreadyToday++
      continue
    }
    ...
    if (wantEmail && RESEND_KEY && r.email) {
      try {
        await sendEmail(RESEND_KEY, RESEND_FROM, r.email, titleText, auto.content || '')
  ...
  if (sent > 0) {
    // Атомарный inc через RPC мы не делали; здесь read-modify-write,
    // race не страшен — счётчик может временно немного отстать, для UI ок.
