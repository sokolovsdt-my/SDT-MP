-- ─── Шаг 4: списание визитов indiv_subscriptions при отметке посещения ───────
-- attendance.subscription_id — FK на subscriptions, для индивов туда писать
-- нельзя. Добавляем отдельную колонку для индив-пакетов и расширяем
-- mark_attendance / cancel_lesson, чтобы они работали с индив-пакетом ровно
-- так же, как с обычным абонементом: present → -1 визит, absent/cancelled
-- из present → +1 визит, отмена урока → возврат всех визитов.
--
-- Подбор пакета: при confirm_indiv_request в indiv_requests.subscription_id
-- уже зафиксирован конкретный пакет — используем его (это ровно тот пакет,
-- что клиент "потратил" при записи). Fallback: свежий активный пакет
-- клиента у этого препода — на случай старых индив-уроков без
-- indiv_requests или если пакет был куплен/выбран уже после записи.

alter table attendance
  add column if not exists indiv_subscription_id uuid references indiv_subscriptions(id);

create index if not exists attendance_indiv_subscription_idx
  on attendance(indiv_subscription_id)
  where indiv_subscription_id is not null;

-- Заодно закрываем дыру advisor'а: cancel_indiv_request была доступна anon.
revoke execute on function public.cancel_indiv_request(uuid, text) from public, anon;
grant  execute on function public.cancel_indiv_request(uuid, text) to authenticated;
...
begin
  ...
  -- present без основания запрещён (для индивов: нет пакета — basis всё равно 'indiv',
  -- этот гард не срабатывает; визиты не списываются, но present разрешён —
  -- сценарий «постоплата/договорной индив»).
  ...
  -- Дельта визитов для обычных подписок
  ...
  -- Дельта визитов для индив-пакетов (та же логика)
...
-- ─── cancel_lesson: возврат визитов и для indiv_subscriptions ────────────────
...
begin
  ...
  -- Возврат визитов: обычные подписки
  for v_att in
  ...
  loop
    ...
    v_refunded_subs   := v_refunded_subs   || v_att.subscription_id;
  end loop;

  -- Возврат визитов: индив-пакеты
  for v_iatt in
  ...
  loop
    ...
    v_refunded_isubs := v_refunded_isubs || v_iatt.indiv_subscription_id;
  end loop;
  ...
  values (p_schedule_id, 'cancelled', v_admin,
          jsonb_build_object(
            'refunded_visits',       v_refunded_visits,
            'refunded_indiv_visits', v_refunded_indiv,
            'removed_payments',      v_removed_payments
          ),
          'Занятие отменено. Возвращено визитов: ' ||
          (v_refunded_visits + v_refunded_indiv) ||
          case when v_removed_payments > 0 then ', отозвана зарплата' else '' end);
