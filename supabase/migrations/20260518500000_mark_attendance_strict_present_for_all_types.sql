-- Универсальный фикс: при переходе ВО present basis пересматривается для
-- ЛЮБОГО типа урока (group/indiv/event), не только для индива. Раньше
-- для group/event ветка «существующая запись с валидным basis сохраняется»
-- слепо переиспользовала старый basis — тот же сценарий что был с индивом:
-- если у клиента когда-то была подписка/регистрация и осталась attendance
-- с basis='subscription'/'event' (но subscription_id=null / event_id у урока
-- изменился / клиент отменил регистрацию), present снова проходил без
-- актуальной проверки.
--
-- Новое правило:
--   * Переход в present (или новая строка) → ВСЕГДА пересматриваем basis
--     заново по lesson_type.
--   * Прочие переходы (present → absent/cancelled/transferred, absent →
--     absent с заметкой и т.п.) при наличии валидной existing записи
--     используют её basis/subscription_id/indiv_subscription_id — это нужно,
--     чтобы корректно вернуть визит в ту подписку, с которой его списали.
--
-- existing_valid: basis ≠ none И соответствующий id для basis типа
-- (subscription/single → subscription_id, indiv → indiv_subscription_id,
-- trial/event → без id).
...
begin
  ...
  -- Валидна ли существующая запись? (для решения «доверять или пересматривать»)
  v_existing_valid := v_att.id is not null
    and v_att.basis is not null
    and v_att.basis <> 'none'
    and (
      v_att.basis in ('trial','event')
      or (v_att.basis in ('subscription','single') and v_att.subscription_id is not null)
      or (v_att.basis = 'indiv' and v_att.indiv_subscription_id is not null)
    );
  ...
  if p_mark_as_trial then
  ...
  elsif p_new_status <> 'present' and v_existing_valid then
    -- Не-present переход с валидной existing — доверяем (нужно для корректного
    -- возврата визита в ту же подписку, с которой списывали).
    v_basis   := v_att.basis;
    ...
    -- present для индива — подбор пакета заново
    ...
    -- present для event — проверяем актуальную регистрацию (а не доверяем
    -- старой attendance.basis='event': клиент мог отменить регистрацию).
    if exists (
    ...
    -- present для групповых — подбор активной подписки заново
    select * into v_sub
