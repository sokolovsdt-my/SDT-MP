-- Догон предыдущего фикса: гард no_valid_basis для индивов не срабатывает,
-- если у клиента УЖЕ есть строка в attendance с basis='indiv' (записанная
-- старым кодом до миграции — там был fallback и indiv_subscription_id мог
-- быть null). Ветка "существующая строка с валидным basis сохраняется"
-- слепо принимает её и пропускает present.
--
-- Решение: для lesson_type='indiv' всегда пересматриваем basis — не доверяем
-- старой записи. Если в indiv_requests.subscription_id или среди активных
-- пакетов клиента у этого препода найдено — basis='indiv', иначе 'none'.
...
begin
  ...
  if p_mark_as_trial then
    ...
    -- Для индивов ВСЕГДА пересматриваем basis (не доверяем старой attendance
    -- с basis='indiv'+null subscription_id — наследие старого fallback'а).
    ...
    -- Для групп/event: существующая строка с валидным basis — доверяем.
    v_basis   := v_att.basis;
