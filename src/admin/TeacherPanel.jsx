...
export default function TeacherPanel({ session }) {
  ...
  const [stats, setStats] = useState({ lessons: 0, students: 0, thisMonth: 0, streak: 0 })
  ...
  const loadAll = async () => {
    ...
    // Расписание своих занятий (30 дней вперёд)
    ...
    const { data: att } = await supabase.from('attendance')
      .select('created_at, student_id, schedule:schedule_id(starts_at)')
      .eq('teacher_id', uid)
      .eq('status', 'present')
    ...
    setStats({ lessons: att?.length || 0, students: uniqueStudents, thisMonth, streak: 0 })

    // Дни рождения учеников своих групп (ближайшие 14 дней)
  ...
  return (
    <div style={{fontFamily:'Inter,sans-serif', background:'#F8F8F8', minHeight:'100vh', maxWidth:480, margin:'0 auto'}}>

      {/* Шапка */}
      <div style={{background:'#2a2a2a', padding:'20px 20px 16px', position:'sticky', top:0, zIndex:10}}>
        <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:16}}>
          <div>
            <div style={{fontSize:13, color:'#888', marginBottom:2}}>{getHour()},</div>
            <div style={{fontSize:20, fontWeight:600, color:'#fff'}}>{myName} 👋</div>
          </div>
          <div style={{display:'flex', alignItems:'center', gap:10}}>
            ...
            <button onClick={() => supabase.auth.signOut()}
              style={{padding:'6px 12px', background:'transparent', border:'1px solid #444', borderRadius:8, fontSize:12, color:'#888', cursor:'pointer', fontFamily:'Inter,sans-serif'}}>
              Выйти
            </button>
          </div>
        </div>
        ...
        <div style={{display:'flex', gap:4}}>
          {[['main','Главная'],['schedule','Расписание'],['tasks','Задачи'],['indivs','Индивы'],['slots','Мои слоты'],['profile','Моя страница']].map(([v,l]) => (
            <div key={v} onClick={() => goTab(v)}
              style={{padding:'6px 10px', borderRadius:8, fontSize:11, cursor:'pointer', fontWeight: tab===v ? 600 : 400,
                background: tab===v ? '#BFD900' : 'transparent', color: tab===v ? '#2a2a2a' : '#888', whiteSpace:'nowrap'}}>
              {l}{v==='tasks' && tasks.length > 0 ? ` (${tasks.length})` : ''}
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{padding:'16px 16px 80px'}}>
        ...
        {tab === 'main' && (
          <div>
                ...
                {attendancePanel === s.id && (
                  <div style={{marginTop:12, paddingTop:12, borderTop:'1px solid #f0f0f0'}}>
                    {(s.bookings || []).filter(b => b.status === 'confirmed').length === 0 ? (
                      ...
                      <div key={b.id} style={{display:'flex', justifyContent:'space-between', alignItems:'center', padding:'6px 0', borderBottom:'1px solid #f8f8f8'}}>
                        ...
                        <div style={{display:'flex', gap:6}}>
                          <button onClick={() => markAttendance(b.id, b.student_id, s.id, 'present')}
                            style={{padding:'4px 10px', background:'#eafaf1', border:'none', borderRadius:6, fontSize:11, color:'#27ae60', cursor:'pointer', fontFamily:'Inter,sans-serif', fontWeight:600}}>
                            ✓ Был
                          </button>
                          <button onClick={() => markAttendance(b.id, b.student_id, s.id, 'absent')}
                            style={{padding:'4px 10px', background:'#fdecea', border:'none', borderRadius:6, fontSize:11, color:'#e74c3c', cursor:'pointer', fontFamily:'Inter,sans-serif', fontWeight:600}}>
                            ✗ Нет
                          </button>
                        </div>
                      </div>
            ...
            {birthdays.length > 0 && (
              <>
                <div style={{fontSize:11, color:'#BDBDBD', textTransform:'uppercase', letterSpacing:'0.08em', marginBottom:8}}>🎂 Дни рождения</div>
                {birthdays.map((p, i) => (
        ...
        {tab === 'schedule' && (
          <div>
            <div style={{display:'flex', gap:6, marginBottom:16}}>
              {[['today','Сегодня'],['week','Неделя'],['month','Месяц']].map(([v,l]) => (
                <button key={v} onClick={() => setScheduleView(v)}
                  style={{padding:'6px 14px', borderRadius:10, border: scheduleView===v ? 'none' : '1px solid #e0e0e0', background: scheduleView===v ? '#BFD900' : '#fff', fontSize:12, cursor:'pointer', fontFamily:'Inter,sans-serif', fontWeight: scheduleView===v ? 600 : 400, color: scheduleView===v ? '#2a2a2a' : '#888'}}>
                  {l}
                </button>
              ))}
            </div>
            ...
            {(() => {
              const list = scheduleView === 'today' ? todaySchedule
                : scheduleView === 'week' ? weekSchedule
                : schedule
            ...
            {tasks.length === 0 ? (
              ...
              <div key={t.id} style={{background:'#fff', borderRadius:14, border:'1px solid #f0f0f0', padding:14, marginBottom:10}}>
                ...
                <div style={{display:'flex', gap:8, alignItems:'center'}}>
                  <span style={{fontSize:11, padding:'2px 8px', borderRadius:6, background:
                    t.status==='new'?'#e8f4fd': t.status==='in_progress'?'#eafaf1': t.status==='problem'?'#fdecea':'#f9f9f9',
                    color: t.status==='new'?'#2980b9': t.status==='in_progress'?'#27ae60': t.status==='problem'?'#e74c3c':'#888', fontWeight:600}}>
              ...
              <div style={{background:'#fff', borderRadius:14, padding:16, marginBottom:16, border:'1px solid #f0f0f0'}}>
                ...
                <div style={{display:'flex', gap:8}}>
                  <button onClick={handleAddSlot}
                    style={{flex:1, padding:'9px', background:'#BFD900', border:'none', borderRadius:8, fontSize:13, fontWeight:700, color:'#2a2a2a', cursor:'pointer', fontFamily:'Inter,sans-serif'}}>
                    Добавить
                  </button>
                  <button onClick={() => setShowAddSlot(false)}
                    style={{padding:'9px 14px', background:'transparent', border:'1px solid #e0e0e0', borderRadius:8, fontSize:12, color:'#888', cursor:'pointer', fontFamily:'Inter,sans-serif'}}>
                    Отмена
                  </button>
                </div>
              </div>
            ...
            {slots.length === 0 ? (
              ...
              return (
                <div key={day} style={{marginBottom:14}}>
                  ...
                  {daySlots.map(s => (
                    <div key={s.id} style={{display:'flex', justifyContent:'space-between', alignItems:'center', background:'#fff', borderRadius:12, padding:'10px 14px', marginBottom:6, border:'1px solid #f0f0f0', opacity: s.is_active ? 1 : 0.5}}>
                      <div style={{fontSize:13, color:'#2a2a2a'}}>{s.start_time.slice(0,5)} — {s.end_time.slice(0,5)}</div>
                      <div style={{display:'flex', gap:8}}>
                        <button onClick={() => handleToggleSlot(s)}
                          style={{fontSize:11, color: s.is_active ? '#27ae60' : '#888', background:'none', border:'none', cursor:'pointer', fontFamily:'Inter,sans-serif', fontWeight:600}}>
        ...
        {tab === 'profile' && (
          <div>
            <div style={{background:'#fff', borderRadius:16, border:'1px solid #f0f0f0', overflow:'hidden', marginBottom:16}}>
              {profile?.avatar_url ? (
                <img src={profile.avatar_url} alt="" style={{width:'100%', height:200, objectFit:'cover'}} />
              ) : (
                <div style={{width:'100%', height:200, background:'linear-gradient(135deg, #2a2a2a, #444)', display:'flex', alignItems:'center', justifyContent:'center'}}>
                  <div style={{width:72, height:72, background:'#BFD900', borderRadius:'50%', display:'flex', alignItems:'center', justifyContent:'center', fontSize:26, fontWeight:700, color:'#2a2a2a'}}>
                    {getName(profile)[0]?.toUpperCase()}
                  </div>
                </div>
              )}
              <div style={{padding:16}}>
                <div style={{fontSize:18, fontWeight:600, color:'#2a2a2a', marginBottom:4}}>{getName(profile)}</div>
                {profile?.bio
            ...
            <div style={{fontSize:12, color:'#BDBDBD', textAlign:'center'}}>
              Так тебя видят ученики в разделе Команда
            </div>
          </div>
        )}
