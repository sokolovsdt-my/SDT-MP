...
export default function AdminDashboard({ session }) {
  ...
  const loadAll = async () => {
    ...
    // Мои задачи
    const { data: assignedTasks } = await supabase.from('task_assignees')
    ...
    if (taskIds.length > 0) {
      const { data: tasksData } = await supabase.from('tasks')
        .select('*')
        .in('id', taskIds)
        .in('status', ['new', 'in_progress', 'postponed', 'problem'])
        .order('created_at', { ascending: false })
        .limit(5)
      setMyTasks(tasksData || [])
    }

    // Дни рождения вчера/сегодня/завтра
    const now2 = new Date()
    ...
    const { data: birthdayClients } = await supabase.from('profiles')
      .select('id, full_name, phone, birth_date')
      .neq('role', 'client')
      .not('birth_date', 'is', null)

    // Также клиентов
    const { data: birthdayClientUsers } = await supabase.from('profiles')
      .select('id, full_name, phone, birth_date, role')
      .eq('role', 'client')
      .not('birth_date', 'is', null)

    const allBirthdayPeople = [...(birthdayClients || []), ...(birthdayClientUsers || [])]
    const bdayList = allBirthdayPeople
      .map(c => {
    ...
    const { data: newClientsData } = await supabase.from('profiles')
      ...
      .eq('role', 'client')
      .gte('created_at', ago7 + 'T00:00:00')
      .order('created_at', { ascending: false })
      .limit(10)
    setNewClients(newClientsData || [])

    // Для teacher — индивы с остатком занятий
    if (role === 'teacher') {
      const { data: indivSubs } = await supabase.from('subscriptions')
        ...
        .gte('expires_at', today)
        .eq('is_frozen', false)
        .not('visits_total', 'is', null)
      // Фильтруем только индивы (у которых есть visits_total)
      const indivData = (indivSubs || []).filter(s =>
        s.type && (s.type.toLowerCase().includes('индив') || s.type.toLowerCase().includes('indiv'))
      )
      setMyIndivs(indivData)
    }
  ...
  return (
    <div>
      ...
      <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:24, flexWrap:'wrap', gap:12}}>
        ...
        <button onClick={() => navigate('/admin/clients?new=1')}
      ...
      {/* Owner финансовые карточки */}
      {userRole === 'owner' && (
        <div style={{display:'grid', gridTemplateColumns:'repeat(4, 1fr)', gap:12, marginBottom:16}}>
          {[
            { label: 'Выручка сегодня', value: fmtMoney(revenueToday), sub: `${salesToday} продаж`, color: '#BFD900', bg: '#fafde8' },
            { label: 'Выручка за месяц', value: fmtMoney(revenueMonth), color: '#6a7700', bg: '#fafde8' },
            { label: 'Активных абонементов', value: activeSubs, color: '#27ae60', bg: '#eafaf1' },
            { label: 'Истекает за 7 дней', value: expiringSoon, color: '#f39c12', bg: '#fef9e7', clickable: true },
          ].map(({ label, value, sub, color, bg, clickable }) => (
            <div key={label}
              onClick={clickable ? () => navigate('/admin/finance') : undefined}
      ...
      <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:16}}>
        ...
        <div>
          ...
          <div style={cardStyle}>
            ...
            {todaySchedule.length === 0 ? (
              ...
              <div key={s.id} style={{display:'flex', gap:12, padding:'10px 0', borderBottom:'1px solid #f8f8f8', alignItems:'center'}}>
                ...
                {s.groups?.color && (
                  <div style={{width:8, height:8, borderRadius:'50%', background:s.groups.color, flexShrink:0}} />
                )}
              </div>
            ))}
          </div>

          {/* Для teacher — индивы с остатком */}
          ...
          {/* Дни рождения */}
          <div style={cardStyle}>
            <div style={{fontSize:14, fontWeight:600, color:'#2a2a2a', marginBottom:14}}>🎂 Дни рождения сегодня</div>
            {birthdays.length === 0 ? (
    ...
    <div key={c.id} style={{display:'flex', justifyContent:'space-between', alignItems:'center', padding:'8px 0', borderBottom:'1px solid #f8f8f8'}}>
      <div>
        ...
        <div style={{fontSize:11, color:'#BDBDBD', marginTop:2}}>
          {c.phone || '—'} · {c.role === 'client' ? 'Клиент' : 'Сотрудник'}
        </div>
      </div>
