...
- **После любой правки объясняй на «студийном» языке, а не техническом.** Когда что-то добавил, поправил, удалил или мигрировал — в ответе пользователю обязательна короткая секция «как было / как стало» на конкретном кейсе из жизни студии. Не «триггер `prevent_invalid_booking` отбивает INSERT при `is_closed=true`», а: «раньше клиент с подходящим абонементом мог записаться даже в закрытую группу через прямой запрос — теперь не может, в приложении на месте кнопки увидит „🔒 Закрыта“. Админ всё равно может записать его вручную из AttendancePanel.» Технический разбор оставляй в commit-сообщении — там нужен для будущего git log. В чате — живой кейс.
- **Явно говори, когда изменение пока ни на что не влияет.** Если добавил поле в БД / новый RPC / триггер, который активируется только после явного действия админа — это нужно проговаривать. Пример: «миграция добавила `groups.age_min/age_max`, но у всех 6 существующих групп они `null` — никаких изменений в поведении не будет, пока ты не зайдёшь в AdminGroups и не заполнишь рамки конкретной группы». Иначе человек переживает что «всё сломали», хотя в реальности изменение спящее.
- **Где можно — приводи реальные данные из БД, а не гипотетику.** Если объясняешь эффект миграции на `groups` — сделай быстрый `select` и покажи сколько групп затронуты и какие. Это снимает 90% беспокойства.
...
- ~~**S15. Задачи без ассайни + без эскалации.**~~ **ЗАКРЫТО.** [AdminTasks.handleSave](src/admin/AdminTasks.jsx) теперь показывает `confirm()` при попытке создать задачу без ответственных — текст «её никто не увидит в фильтре „Мои задачи“ и эскалация не сработает», админ может всё равно подтвердить. Эскалация: таблица `task_overdue_notifications(task_id, assignee_id, run_date)` с `UNIQUE` для anti-spam, edge function `notify-overdue-tasks` (auth по cron-secret либо admin JWT), pg_cron job `notify-overdue-tasks-hourly` (`0 * * * *`) → `process_overdue_tasks()` через `net.http_post` дёргает edge. Edge берёт `tasks` где `deadline < МСК now()` и `status NOT IN (done,closed,cancelled)`, для каждого `task_assignees`-пэйра вставляет run-row (`ON CONFLICT` → пропуск), шлёт FCM «⏰ Просроченная задача: ... (дедлайн ДД.ММ ЧЧ:ММ)». Без `push_token` ставит `error='no_push_token'` в run-row.
- ~~**STAFF-whitelist на клиенте.**~~ **ЗАКРЫТО (DB-уровень).** Создана таблица [public.staff_whitelist](supabase/migrations/20260519130000_audit_p1_partE_remaining.sql) (email PK, added_at, added_by, comment) — единый источник правды. RLS: SELECT для всех authenticated/anon (UI Login читает), write только manager/owner. Сид: 2 email из старого хардкода + auto-import всех живых staff из `profiles WHERE role IN (admin,manager,owner,teacher)`. [App.jsx Login.handleMagicLink](src/App.jsx) вместо хардкода `STAFF` делает запрос `staff_whitelist.ilike(email)` — даёт понятную ошибку «Сотрудники входят только по паролю». Серверный гард: триггер `profiles_staff_password_check` BEFORE INSERT на `profiles` — если email в whitelist и `auth.users.encrypted_password IS NULL` → `raise 'staff_email_requires_password'`. **TODO (полная защита):** включить в Supabase Dashboard **Before-User-Created Auth Hook** на ту же проверку — тогда блокировка будет до создания auth.users, а не до записи в profiles. Без хука сценарий «auth.users создан без profile» теоретически возможен (хотя реального flow нет).
- **Индив-flow: уведомлений нет.** Edge или trigger на `indiv_requests` INSERT/UPDATE → push преподу/клиенту.
- ~~**AdminFinance loyalty/groups attendance без даты-фильтра.**~~ **ЗАКРЫТО (точечный фикс).** [AdminFinance.jsx](src/admin/AdminFinance.jsx) теперь добавляет `.gte('marked_at', mskDayStartUtc(ago90Iso))` — берём только последние 90 дней `attendance.marked_at` со `status='present'`. Этого хватает для расчёта «не ходят 10+ дней». На длинной дистанции (если понадобится >90 дней) — RPC `loyalty_last_visits` с агрегацией. Отложено в Трек 4.
- ~~**TeacherPanel: `attendance`/`bookings` без лимита.**~~ **ЗАКРЫТО.** Новая RPC `teacher_stats(p_month_start date default null)` агрегирует в БД через `count(*) / count(distinct student_id) / count(*) filter (created_at >= month_start)` по `attendance WHERE teacher_id=auth.uid() AND status='present'`. Возвращает `{ok, lessons, students, this_month, month_start}`. [TeacherPanel.jsx](src/admin/TeacherPanel.jsx) теперь делает один RPC-вызов вместо тяги всей истории на клиент. Доступ — `teacher/admin/manager/owner`. ДР учеников по группам пока остаётся прежним запросом — это P3, объёмы меньше.

### 🟡 P2 — средние, плановые улучшения

- ~~**S16. `is_closed` группы не проверяется при клиентской записи.**~~ **ЗАКРЫТО (Партия H).** Триггер [`prevent_invalid_booking`](supabase/migrations/20260519150000_audit_p2_partH_groups_age_indiv_cleanup.sql) BEFORE INSERT на `bookings`: если `current_user='authenticated'` (прямой клиентский INSERT), читает `schedule.group_id → groups.is_closed` и при `true` отбивает `group_closed`. Админы пропускаются (для записи руками в AttendancePanel). UI [`Schedule.handleBook`](src/pages/Schedule.jsx) сначала вызывает `bookingBlocker(cls)` — клиент видит понятную надпись «🔒 Закрыта» вместо кнопки «Записаться» (с title-hint).
- ~~**S17. Возрастная валидация.**~~ **ЗАКРЫТО (Партия H).** Миграция добавила `groups.age_min int, age_max int`. Триггер `prevent_invalid_booking` считает возраст клиента из `profiles.birth_date` через `extract(year from age(...))` и блокирует записи с `age_below_min`/`age_above_max`. UI [`Schedule.bookingBlocker`](src/pages/Schedule.jsx) дублирует проверку: если у клиента не заполнен `birth_date` и у группы есть возрастные рамки — просит сначала заполнить ДР в профиле; иначе вместо кнопки показывает бейдж «👶 X–Y лет». [`AdminGroups`](src/admin/AdminGroups.jsx) расширен полями «Возрастной диапазон» (два number-input) + валидация min ≤ max + бейдж возраста в списке групп.
- ~~**S18. Истёкшие `pending` индив-заявки занимают слот.**~~ **ЗАКРЫТО (Партия H).** RPC `process_stale_indiv_requests()` обновляет `indiv_requests` со `status='pending' AND created_at < now() - 48h` → `status='cancelled'`, `reject_reason='Истекла без подтверждения (48ч)'`. partial UNIQUE `(teacher_id, slot_date, start_time) WHERE status IN ('pending','confirmed')` сам освобождает слот. pg_cron job `cleanup-stale-indiv-requests-hourly` дёргает раз в час в 15-ю минуту (рассинхронизация с другими hourly-job'ами).
...
- ~~**S20. ILIKE-поиск без trgm-индекса.**~~ **ЗАКРЫТО (Партия G).** Включено расширение `pg_trgm`, созданы два GIN-индекса: `profiles_search_trgm_idx` на `(full_name||' '||email||' '||phone)` (покрывает .or(...ilike) в AdminCashbox/AdminBroadcasts/AdminClientCard search) и `profiles_full_name_trgm_idx` на `full_name` (для одиночных .ilike в AttendancePanel). На студии 1000+ клиентов поиск ILIKE ускоряется в 10-100× — PostgreSQL может использовать индекс для wildcard'ов на обоих сторонах (`%val%`).
...
- ~~**S22. PostgREST `.or(...ilike.%${val}%)` без escape.**~~ **ЗАКРЫТО (Партия G).** Добавлена утилита [`escapeIlike(value)`](src/utils/postgrest.js) — экранирует `%`, `_`, `\`, `,`, `(`, `)`, `*`, `'` для безопасного использования внутри `.or()` и `.ilike()`. Применена в [AdminCashbox.ClientSearch](src/admin/AdminCashbox.jsx), [AdminBroadcasts.handleManualSearch](src/admin/AdminBroadcasts.jsx) и [AttendancePanel student search](src/admin/AttendancePanel.jsx). Теперь поиск «O'Leary», «100%», «name, surname» не ломает PostgREST-парсер и не превращает пользовательский ввод в wildcards.
- ~~**S23. Картинки без `loading="lazy"`.**~~ **ЗАКРЫТО (Партия G).** Bulk-добавлены атрибуты `loading="lazy" decoding="async"` на все 21 `<img>` в src/* (AdminPrizes, AdminCatalog, TeacherPanel, AdminClients, AvatarUpload, Home, Shop, Bonus, Team). Браузер откладывает загрузку изображений за пределами viewport — экономия мобильного трафика. **Supabase Image Transformations** (`?width=N&quality=80`) — отложены, т.к. требуют Pro plan.
- ~~**S24. Контраст `#BFD900` на белом + Inter не подгружен.**~~ **ЗАКРЫТО (Партия J).** [index.html](index.html) подгружает Inter (`weight 300/400/500/600/700`) с Google Fonts через preconnect + stylesheet — `fontFamily:'Inter,sans-serif'` теперь действительно использует Inter. Все `color:'#BFD900'` для **текста** заменены на `color:'#6a7700'` (контраст 6.8:1 vs 1.7:1, WCAG AA pass) в Home.jsx (приветствие, статы, стрелка к бонусам, ссылка на новости), News.jsx (toggle «Читать далее»), Schedule.jsx (бейдж группы), AdminDashboard.jsx (KPI «Выручка сегодня», число новых клиентов), AdminCashbox.jsx (итог суммы), AdminBroadcasts.jsx (счётчик получателей), TeacherPanel.jsx (счётчик занятий), AdminFinance.jsx (KPI «Выручка»). Не тронуты: `background:'#BFD900'` (кнопки и бейджи — фон), `color_hex/badge_color/tag_color` (палитра user-выбора), `border:'1.5px solid #BFD900'` (рамки limey-карточек).
- ~~**S25. SW без `notificationclick`.**~~ **ЗАКРЫТО (Партия G).** В [firebase-messaging-sw.js](public/firebase-messaging-sw.js) добавлен handler `notificationclick`: при клике закрывает уведомление, ищет открытую вкладку приложения через `clients.matchAll({type:'window'})`, фокусирует и навигирует на `event.notification.data.url`. Если приложение не открыто — `clients.openWindow(targetUrl)`. `onBackgroundMessage` теперь пробрасывает `payload.data` в `showNotification(..., {data})`, чтобы handler знал URL. Заодно иконка пуша поменяна с `/favicon.ico` на `/icon-192.png` для лучшего отображения на Android.
...
- **Трек 2 «Замкнуть бизнес-логику»:** ✅ S6/S7/S11/S12 (Партия A — RPC `cancel_booking`/`request_prize`/`register_push_token`/`update_my_profile` + два триггера-защитника). ✅ S8/S9 (Партия B — пропорциональный возврат бонусов в `cancel_sale`, конверсия визита в `bonus_rubles` на мёртвой подписке через обновлённый `_refund_visits_for_lesson`). ✅ S15 + STAFF-whitelist + AdminFinance loyalty-фильтр + RPC `teacher_stats` (Партия E). Осталось: **S10 (рефералку доделываем до боевого режима, не удалять UI)**, уведомления для индивов, онлайн-оплата.
- **Трек 3 «Производительность и UX»:** ✅ S13/S14 (Партия D), AdminFinance loyalty с date-фильтром, TeacherPanel через RPC teacher_stats (Партия E), S20/S22/S23/S25 (Партия G), S24 (Партия J). **Трек 3 закрыт полностью.**
- **Трек 4 «Технический долг» (3-4 недели фоном):** S19, S21, очистка дубликатов и мёртвых таблиц (`staff_salary_settings`, `product_indivs`, `products(type='indiv')`); React-hooks рефакторинг + включить `react-hooks/exhaustive-deps` уровнем `error`; смоук-тесты денежного контура через `do $$ assert ... $$`.
