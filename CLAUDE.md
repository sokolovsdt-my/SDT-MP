...
- **Индив-flow: визиты `indiv_subscriptions` не списываются.** `mark_attendance` для `lesson_type='indiv'` ставит `basis='indiv'` с `subscription_id=null` — `attendance.subscription_id` это FK на `subscriptions`, а пакет лежит в `indiv_subscriptions`. Чтобы списывать визиты, нужно либо добавить колонку `attendance.indiv_subscription_id` (FK на `indiv_subscriptions`), либо снять FK с `subscription_id` и интерпретировать по `basis`. После исправления продажа пакета → запись → проведение → списание визита замкнётся end-to-end. *(Шаги 1-3 закрыты, это остаток — см. ниже.)*
- **Индив-flow: занятые слоты не фильтруются в UI.** `Shop.jsx`/`Team.jsx` показывают все активные слоты препода — клиент может нажать «Записаться» на уже занятый и получить `slot_taken` от `create_indiv_request`. Лечится фильтром в `loadTeacher*`: `NOT EXISTS (select 1 from indiv_requests where teacher_id=… and slot_date=… and start_time=… and status in ('pending','confirmed'))`. БД-защита уже стоит (partial UNIQUE), это только UX.
