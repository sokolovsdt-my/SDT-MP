...
- ~~**S10. Реферальная ссылка — fake-feature.**~~ **ЗАКРЫТО (через удаление UI).** Компонент `Referral` и пункт меню «Привести друга ✦» в [Profile.jsx](src/pages/Profile.jsx) убраны, ветка `screen==='referral'` тоже снята. UI больше не обещает несуществующее начисление. Реализация (миграция `profiles.referrer_id` + парсинг `?ref=` при signup + RPC `apply_referral_bonus`) — отложена до момента, когда появится бизнес-кейс. Push-banner остаётся доступен в [Schedule.jsx](src/pages/Schedule.jsx) (показывается после первой брони).
...
- **Трек 2 «Замкнуть бизнес-логику»:** ✅ S6/S7/S11/S12 (Партия A — RPC `cancel_booking`/`request_prize`/`register_push_token`/`update_my_profile` + два триггера-защитника). ✅ S8/S9 (Партия B — пропорциональный возврат бонусов в `cancel_sale`, конверсия визита в `bonus_rubles` на мёртвой подписке через обновлённый `_refund_visits_for_lesson`). ✅ S10 (Партия C — fake-фича «Привести друга» снята из UI). Осталось: S15 (задачи без ассайни + эскалация), уведомления для индивов + онлайн-оплата.
