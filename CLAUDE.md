...
- **ESLint flat config: преэкзистирующий React-hooks долг (~15-25 ошибок).** Подключён `eslint-plugin-react-hooks` (см. [eslint.config.js](eslint.config.js)), но три категории нарушений накопились в больших файлах и проходят `npm run lint` как warnings, не падая. Что именно ругается:
  - **`react-hooks/purity`** — вызовы impure-функций в теле компонента (`Date.now()`, `Math.random()`, `Math.min(...emptyArray) === Infinity`, чтение `window`/`localStorage` напрямую). Пример: [Team.jsx:57](src/pages/Team.jsx:57) `new Date(Date.now() + 30*24*60*60*1000)`. Чинить: переносить в `useEffect` / `useMemo` / `useState(() => ...)`.
  - **`react-hooks/exhaustive-deps`** — `useEffect(fn, [])` где `fn` использует значения из замыкания. Пример: [TeacherPanel.jsx:454](src/admin/TeacherPanel.jsx:454) `useEffect(() => { loadAll() }, [])`. Чинить: оборачивать handler-функции в `useCallback` + добавлять в deps, либо явный `eslint-disable-next-line` с комментарием «зависимости стабильны».
  - **`react-hooks/immutability`** — функция используется в `useEffect`/JSX до её объявления (например, [Team.jsx:30](src/pages/Team.jsx:30) — `loadTeacher(selected)` в `useEffect` выше `const loadTeacher`). Хоистинг работает в JS, но React 19 ругается на риск устаревания ссылки. Чинить: переставлять объявления выше использования.
  - **Когда я говорю «новых ошибок нет»** в commit message — это означает что число и список ошибок до и после моих правок одинаковые. Эти warning'и не падают в проде (большинство «работает случайно правильно» из-за стабильных зависимостей), но накапливаются.
  - **План фикса (Трек 4):** отдельный refactoring-PR — пройтись по всем `useEffect`/handler'ам, добавить `useCallback`/`useMemo`, исправить impure-вызовы. После — включить `react-hooks/exhaustive-deps` уровнем `error` в eslint.config.js, чтобы новые ошибки больше не накапливались.
