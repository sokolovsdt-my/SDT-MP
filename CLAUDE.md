...
- **teacher_indiv_slots** — `id, teacher_id, day_of_week, start_time, end_time, is_active`. **Мёртвая таблица**, в коде не используется. Не путать с `teacher_slot_dates`.
- **teacher_slot_dates** — `id, teacher_id, date, start_time, end_time, is_active`. Реальные слоты на 30 дней; читают Shop/Team, пишут AdminCatalog SlotsSection и TeacherPanel SlotsCalendar.
