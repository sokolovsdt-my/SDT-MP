...
| `mark_attendance(p_schedule_id, p_student_id, p_new_status, p_mark_as_trial?)` | Отметка посещаемости в [AttendancePanel](src/admin/AttendancePanel.jsx), [TeacherPanel](src/admin/TeacherPanel.jsx) и при переносе пробного в `TransferModal`. `p_mark_as_trial=true` — из `NoBasisModal` для пробных в групповых уроках | UPSERT в `attendance` через `ON CONFLICT`. Определение `basis` по приоритету: `p_mark_as_trial=true` → `trial`; **для `lesson_type='indiv'` basis ВСЕГДА пересматривается заново** (через `indiv_requests.subscription_id` для этого `schedule_id`, fallback — свежий активный пакет клиента у препода) — старой записи не доверяем, иначе унаследованная basis='indiv' от прежнего fallback'а ломала бы гард; найден → `indiv`, не найден → `none`. Для групп/event: существующая строка с валидным basis сохраняется. `lesson_type='event'` → лукап `event_registrations` (зарегистрирован → `event`, иначе `none`); иначе — подбор активного абонемента под группу (`subscription`/`single`/`none`). **Гард `no_valid_basis`:** `present` с `basis='none'` отклоняется для **любого** типа урока. В ответ ошибки идёт `lesson_type` — клиент показывает «Нет оплаченного пакета на индив» для индив-уроков и общий текст «Нет действующего основания…» для прочих. При переходе в/из `present` корректирует `subscriptions.visits_used` ИЛИ `indiv_subscriptions.visits_used` атомарно (зависит от basis), с верхней границей: `out_of_visits` для обычных, `indiv_out_of_visits` для индив-пакетов. Учитель допускается только к своему/подменному уроку. Возврат включает `indiv_subscription_id`, `indiv_visits_used`, `indiv_visits_total`, `indiv_delta`. |
| `cancel_lesson(p_schedule_id)` | «✕ Отменить занятие» в [AttendancePanel](src/admin/AttendancePanel.jsx) | Помечает урок `is_cancelled=true`, возвращает визиты всем `present`-ученикам — отдельно в `subscriptions` (по `attendance.subscription_id`) и в `indiv_subscriptions` (по `attendance.indiv_subscription_id`), удаляет `lesson_payments` за этот урок, пишет в `schedule_history`. В ответ: `refunded_visits` (обычные), `refunded_indiv_visits` (индив), массивы id обоих типов подписок. |
...
- **Гард «занятие уже началось»**: в `AttendancePanel` кнопка «✕ Отменить» disabled (текст «✕ Уже началось» + title-подсказка) если `parseMskNaive(starts_at) <= now()` — отменять активный/прошедший урок задним числом нельзя (для пришедших учеников «отмена» = отъём оплаченного визита, плюс зарплата уже могла быть подтверждена). Аналогично в [TeacherIndivDetail](src/components/TeacherIndivDetail.jsx) для клиента — кнопка «Отменить» рядом с заявкой скрывается, если `parseMskNaive(slot_date+T+start_time) <= now()` (фильтр БД по `slot_date >= today` грубый: если время на сегодня уже прошло, заявка остаётся видна). В `Profile.MyLessons` дополнительной защиты не нужно — там upcoming фильтруется минутным `parseMskNaive(starts_at) >= now`. Это UI-правила, сервер `cancel_lesson`/`cancel_indiv_request` оставлены открытыми (12h-барьер для `cancel_indiv_request` сохраняется).
...
Бэклог из аудита на момент `577c4a4`. Закрытые пункты не перечисляются — историю смотри в `git log` по префиксам выше. Ссылки на строки могут смещаться, но имена функций/файлов стабильны.
